...
class MotivationAnswerDto {
  ...
  @Transform(({ value }) =>
    value ? String(value).trim().substring(0, 500) : null,
  )
  ...
  @Transform(({ value }) => {
    ...
    if (Array.isArray(value.learningStyles)) {
      sanitized.learningStyles = value.learningStyles
        .filter(
          (s: any) =>
            typeof s === 'string' && ALLOWED_LEARNING_STYLES.includes(s as any),
        )
    ...
    if (
      typeof value.gamification === 'string' &&
    ...
    if (
      typeof value.sessionStyle === 'string' &&
