...
export class OnboardingController {
  ...
  @ApiResponse({ status: 200, description: 'Onboarding saved successfully', type: OnboardingResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Onboarding retrieved', type: OnboardingResponseDto })
  @ApiResponse({ status: 404, description: 'Onboarding not found' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async getOnboarding(@User() userId: string): Promise<OnboardingResponseDto | null> {
    return this.onboardingService.getOnboarding(userId);
  }

  @Get('has')
  @ApiOperation({ summary: 'Check if user has completed onboarding' })
  @ApiResponse({ status: 200, description: 'Onboarding status retrieved', type: HasOnboardingResponseDto })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async hasOnboarding(@User() userId: string): Promise<HasOnboardingResponseDto> {
