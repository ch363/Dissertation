...
import { sanitizeBase64, sanitizeString, sanitizeEnum } from '../../common/utils/sanitize.util';
...
export class ValidatePronunciationDto {
  ...
  @Matches(/^[a-z0-9]+$/i, { message: 'Audio format must contain only alphanumeric characters' })
  @Transform(({ value }) => sanitizeEnum(value?.toLowerCase(), ALLOWED_AUDIO_FORMATS) ?? sanitizeString(value?.toLowerCase(), 10))
  audioFormat: string;
}
