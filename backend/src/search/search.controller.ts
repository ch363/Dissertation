...
export class SearchController {
  ...
  @Get()
  @ApiOperation({ summary: 'Search modules, lessons, teachings, and questions' })
  @ApiQuery({ name: 'q', type: 'string', required: false, description: 'Search query text' })
  @ApiQuery({ name: 'level', enum: ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'], required: false })
  @ApiQuery({ name: 'topic', type: 'string', required: false })
  @ApiQuery({ name: 'type', enum: ['module', 'lesson', 'teaching', 'question'], required: false })
