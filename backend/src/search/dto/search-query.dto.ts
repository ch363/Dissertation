...
import { sanitizeString, sanitizeEnum, sanitizeInt } from '../../common/utils/sanitize.util';
 ...
 */
const ALLOWED_SEARCH_TYPES = ['module', 'lesson', 'teaching', 'question'] as const;
...
export class SearchQueryDto {
  ...
  @IsOptional()
  @IsEnum(KNOWLEDGE_LEVEL, { message: 'Knowledge level must be a valid CEFR level (A1-C2)' })
  @Transform(({ value }) => value ? sanitizeEnum(value, Object.values(KNOWLEDGE_LEVEL)) : undefined)
  ...
  @IsOptional()
  @IsString()
  @IsEnum(ALLOWED_SEARCH_TYPES, { message: 'Type must be one of: module, lesson, teaching, question' })
  @Transform(({ value }) => value ? sanitizeEnum(value?.toLowerCase(), ALLOWED_SEARCH_TYPES) : undefined)
  ...
  @Transform(({ value }) => value !== undefined ? sanitizeInt(value, 1, 100) : undefined)
  limit?: number;
