  ...
  handleRequest(err: any, user: any, info: any, context: ExecutionContext) {
    ...
    if (!user) {
      ...
      if (!authHeader) {
        ...
        throw new UnauthorizedException('Missing Authorization header. Please provide: Authorization: Bearer <token>');
      }
      ...
      if (info?.message === 'invalid signature') {
        ...
        this.logger.error('1. SUPABASE_JWT_SECRET in .env does not match your Supabase project JWT secret');
        this.logger.error('2. The token was not issued by Supabase (e.g., custom token or wrong project)');
      ...
      this.logger.warn('Token validation failed:', info?.message || 'Unknown error');
      throw new UnauthorizedException(
        info?.message || 'Invalid or expired authentication token. Please check your JWT token and SUPABASE_JWT_SECRET.'
      );
    }
