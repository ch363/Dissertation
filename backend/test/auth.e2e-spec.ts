...
import { generateTestJwt, generateExpiredJwt, generateJwtWithoutSub } from './helpers/jwt.helper';
...
if (!process.env.SUPABASE_JWT_SECRET) {
  process.env.SUPABASE_JWT_SECRET = 'test-jwt-secret-for-e2e-tests-only-do-not-use-in-production';
}
...
describe('Auth (e2e)', () => {
  ...
  beforeEach(async () => {
    try {
      ...
      if (!httpServer) {
        throw new Error('App HTTP server not initialized - app.init() may have failed silently');
      }
    } catch (error) {
