...
- `app/_layout.tsx` continues to wrap `AppProviders`; nav/tab layouts stay in `app/nav-bar`.
...
- `app/nav-bar/profile` + `app/profile/*` → `features/profile/screens/*` (+ progress/achievements)
- `app/nav-bar/settings` + `app/settings/*` → `features/settings/screens/*`
