import { View, Text, StyleSheet } from 'react-native';
import { router } from 'expo-router';
import { theme } from '../../src/theme';
import { Stepper, Option, PrimaryButton, StickyCTA, WhyWeAskLink } from './_components';
import { useOnboarding } from '../../src/onboarding/OnboardingContext';

export default function Tone() {
  const { answers, setAnswerAndSave } = useOnboarding();
  const selected = answers.tone ?? null;

  const options = [
    { key: 'friendly', icon: '😊', label: 'Friendly' },
    { key: 'professional', icon: '💼', label: 'Professional' },
    { key: 'playful', icon: '🎉', label: 'Playful' },
  ];

  const onNext = () => router.push('/onboarding/experience-level');
  const onSkip = () => router.push('/onboarding/experience-level');

  return (
    <View style={styles.container}>
      <Stepper current={8} total={9} />
      <Text style={styles.title}>Tone of voice</Text>
      <WhyWeAskLink />
      {options.map((o) => (
        <Option key={o.key} label={o.label} selected={selected === o.key} onPress={() => setAnswerAndSave('tone', o.key)} icon={o.icon} />
      ))}
      <StickyCTA>
        <PrimaryButton title="Next" onPress={onNext} disabled={!selected} />
        <View style={{ height: 8 }} />
        <PrimaryButton title="Skip / Not sure" onPress={onSkip} />
      </StickyCTA>
    </View>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: theme.colors.background,
    padding: theme.spacing.lg,
  },
  title: {
    fontFamily: theme.typography.semiBold,
    fontSize: 22,
    color: theme.colors.text,
    marginBottom: theme.spacing.lg,
  },
  // next button styles moved to shared components
});
